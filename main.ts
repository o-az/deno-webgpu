...
class SimpleRenderer {
  ...
  saveRenderedImage(data: Uint8Array, width: number, height: number) {
    ...
    for (let i = 0; i < 20; i += 4) {
      console.log(
        `Pixel ${i / 4}: R=${data[i]}, G=${data[i + 1]}, B=${data[i + 2]}, A=${
          data[i + 3]
        }`,
      );
    }
    ...
    const _pngBuffer = PNG.sync.write(png);
    //await Deno.writeFile(`rendered_image_${this.frameCount}.png`, pngBuffer);
    console.log(
      `Rendered image saved as rendered_image_${this.frameCount}.png`,
    );
  }
}
